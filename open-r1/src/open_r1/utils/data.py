...
def get_dataset(args: ScriptArguments) -> DatasetDict:
    ...
    """
    if args.dataset_name.endswith('.json') and os.path.isfile(args.dataset_name):
        ...
        dataset = DatasetDict({"train": Dataset.from_list(new_data)})
        # print(dataset)
        # exit(0)
        return dataset
    elif args.dataset_name and not args.dataset_mixture:
        logger.info(f"Loading dataset: {args.dataset_name}")
        # print(f"Dataset name: {args.dataset_name}, config: {args.dataset_config}")
        # print(datasets.load_dataset(args.dataset_name, args.dataset_config))
        # exit(0)
